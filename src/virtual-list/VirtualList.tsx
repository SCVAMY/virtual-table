  ...
  setup(props) {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        range[1] =
          range[0] + Math.floor(root.value.clientHeight / itemSize.value) + poolBuffer.value
        range[1] = Math.min(range[1], data.value.length)

        pool.value = data.value
          .slice(range[0], range[1])
          .map((v: any, i) => ({ ...v, _index: range[0] + i }))
    ...
    return (
      <div class="virtual-list" ref="root" onScroll={this.handleScroll}>
        <div
          ...
          style={`height: ${this.scrollHeight}px;padding-top: ${this.paddingTop}px`}
        >
